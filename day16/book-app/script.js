...
function removeBook(button) {
    ...
    const index = books.findIndex((element) => element.title === title);
...
function processBooks() {
    ...
    const prefixedBooks = books.map((book) => {
        return { ...book, title: `Book: ${book.title}` };
    });

    // 10000원 이상 도서
    const highPriceBooks = books.filter(({ price }) => price >= 10000);

    // 총 가격 계산
    const totalPrice = books.reduce((acc, { price }) => {
        return acc + price;
    }, 0);
...
function toggleRental(button) {
    ...
    const rental = rentals.find((element) => {
        return element.getStatus().title === title;
    });
    if (!rental) return;
